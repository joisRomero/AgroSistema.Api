IF EXISTS (SELECT * FROM sys.objects WHERE TYPE = 'P' AND name = 'sp_reporteInicio_campaniaTerminada_x_usuario')
	DROP PROCEDURE sp_reporteInicio_campaniaTerminada_x_usuario
GO

CREATE PROCEDURE sp_reporteInicio_campaniaTerminada_x_usuario(
	@p_id_usu INT
)
AS
BEGIN
	...
	IF OBJECT_ID('tempdb..#temp_TablaCampaniaTerminada') IS NOT NULL
		DROP TABLE #temp_TablaCampaniaTerminada
	...
	SELECT 
		...
		,nombre_camp
		,TotalGasto
	INTO #temp_TablaCampaniaTerminada
	FROM GastosCampania
	...
	FROM #temp_TablaCampaniaTerminada
